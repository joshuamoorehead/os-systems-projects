...
int Server::RunServer() {
  ...
  // open shared memory with read/write rights and get file descriptor after client signals
  int shm_fd = ::shm_open(shm_name, O_RDWR, 0);
  if (shm_fd < 0) {
    std::cerr << "Server::Server ::shm_open: SERVER SM OPEN FAILED" << ::strerror(errno)
      << std::endl;
    return errno;
  }
  ...
  if (store == MAP_FAILED) {
    std::cerr << "Server::Server ::mmap: COPY OF MMAP FAILED" << ::strerror(errno)
      << std::endl;
    return errno;
  }
  ...
  char* csv_in_memory = static_cast<char*>(::mmap(nullptr, csv_size.st_size, PROT_READ, MAP_PRIVATE, csv_fd, 0));
