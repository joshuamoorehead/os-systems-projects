      ...
      client_sem(SNAME) , server_sem(SNAME2)
{
  ...
  if (shm_fd < 0) {
    DestroyMemory();
    std::cerr << "Consumer::Consumer --- ::shm_open(CLIENT CAN'T OPEN SHARED MEMORY): " << shm_name_ << ") --- "
...
// set size of shared memory with file descriptor
  ...
  store_ = static_cast<Shared_Memory_Store<kSharedMemSize>*>( //Essentially subbing in 4096 for buffer size
...
void Client::ThreadProcess(std::vector<std::string>& v, size_t start, size_t end,
 std::vector<std::string>& OpsearchTerms, std::vector<std::string>& printVector) {
  ...
  for(size_t i = start; i < end; ++i) {
    ...
    if (line.compare("INVALID FILE") == 0) { //server deemed it invalid
...
void Client::RunClient(std::string file, std::vector<std::string> searchterms)  {
  ...
  std::vector v = explode(file_data, kEoL);//puts data string into vector based on delim value, pulled from c++ website
  ...
  for(int i =0; i<4; i++)
  {
    threads.push_back(std::thread(&Client::ThreadProcess, this, std::ref(v), 
    start_index, end_index, std::ref(searchterms),  std::ref(printVector)));
...
std::string parseCommandLine(int argc, char** argv) {
  ...
  for(int i = 3; i < argc; i++) {//adding all the opcode now to the message
    if(((strcmp(argv[i], "+") == 0 ) || strcmp(argv[i], "x") == 0) && counter == 0) {
